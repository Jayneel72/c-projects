...
public:
    LibraryItem(const string &t = "", const string &a = "") : title(t), author(a), dueDate(""), checkedOut(false) {}
...
public:
    ...
    {
        if (copies_ < 0)
            throw invalid_argument("Copies cannot be negative");
        if (!isbn_.empty() && !isValidISBN(isbn_))
